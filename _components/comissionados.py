...
def registrar_callbacks_comissionados(app):
    ...
    def atualizar_graficos_comissionados(data):
        ...
        for i in range(len(df_raw)):
            row = df_raw.iloc[i]
            if isinstance(row[0], str) and "Período:" in row[0] and i != 6:
                periodo = row[1]
        ...
        styles = [{'display': 'none'} if (df[col].fillna(0) == 0).all() else {} for _, col, *_ in specs]
