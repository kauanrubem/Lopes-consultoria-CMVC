...
def layout_comissionados():
    return dbc.Row([
        *[
            dbc.Col(dbc.Card(dbc.CardBody([dcc.Graph(id=f'fig{i}_comissionados')])),
                    id=f'col{i}_comissionados', xs=12, md=6)
            for i in range(10)
        ]
    ])

def registrar_callbacks_comissionados(app):
    @app.callback(
        [Output(f'fig{i}_comissionados', 'figure') for i in range(10)] +
        [Output(f'col{i}_comissionados', 'style') for i in range(10)],
        Input('data-store', 'data')
    )
    def atualizar_graficos_comissionados(data):
        df_raw = pd.DataFrame(data)
        registros = []

        # Identificar os dados de janeiro diretamente na linha 6 (linha correta para comissionados)
        janeiro_dados = df_raw.iloc[6]  # Pega os dados da linha 6 para Janeiro
        ...
        registros.append({
            "Período": periodo,
            "Status": status,
            "Lotes": "Lote 06 - Comissionados",  # Usando o Lote 06
        ...
        df = df[df['Lotes'] == 'Lote 06 - Comissionados'].reset_index(drop=True)
        ...
        # Preenchendo o mês de Janeiro com os dados da linha 6 e garantindo o status "REALIZADO"
        ...
        specs = [
            ...
            ('Quantidade de Comissionados por Mês', 'Qtd',                   'orange',  '#FFCC80', False),
